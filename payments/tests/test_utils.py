...
class ComputeExpectedAmountTests(TestCase):
    ...
    def test_correct_amount(self):
        ...
        amount = compute_expected_amount(student)
        # 18 * 500 + 200 + 100 = 9300
        self.assertEqual(amount, Decimal("9300"))
...
class AuditLogIntegrityTests(TestCase):
    def setUp(self):
        ...
        self.log = PaymentAuditLog.objects.filter(payment=self.payment).first()

    def test_audit_log_create_still_works(self):
        self.assertIsNotNone(self.log)

    def test_audit_log_update_blocked(self):
        from django.db.models.deletion import ProtectedError

        self.log.actor = "tamper"
    ...
    def test_audit_log_queryset_delete_blocked(self):
        ...
        with self.assertRaises(ProtectedError):
            PaymentAuditLog.objects.filter(payment=self.payment).delete()

    def test_audit_log_queryset_update_blocked(self):
        from django.db.models.deletion import ProtectedError

        with self.assertRaises(ProtectedError):
            PaymentAuditLog.objects.filter(pk=self.log.pk).update(actor="tamper")

    def test_audit_log_bulk_update_blocked(self):
        from django.db.models.deletion import ProtectedError

        self.log.actor = "tamper"
        with self.assertRaises(ProtectedError):
            PaymentAuditLog.objects.bulk_update([self.log], ["actor"])
